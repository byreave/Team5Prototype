...
export default class PlanetGenerator {
    ...
    AutoGenerateAScreen() {
        for (var i = this.XStep; i < this.XSpan; i += this.XStep) {
            ...
            var radius =
                Math.floor(Math.random() * (this.radiusMax - this.radiusMin)) +
                this.radiusMin;
            var texture = this.texturePack[
                Math.floor(Math.random() * this.texturePack.length)
            ];
            var newPlanet = this.GenerateOnePlanet(
                x,
                y,
                radius,
                texture,
                "Planet" + this.totalNumber.toString()
            );
            this.totalNumber++;
