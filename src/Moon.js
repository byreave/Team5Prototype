    ...
    OrbitUpdate(
        ...
        if (this.isOrbiting) {
            ...
            if (this.angle <= Math.PI / 2 && this.angle >= 0) {
                ...
                this.sprite.setX(
                    this.orbit.sprite.x -
                    Math.sin(this.angle - Math.PI / 2) * this.orbit.gravityCircle.radius
                );
                this.sprite.setY(
                    this.orbit.sprite.y -
                    Math.cos(this.angle - Math.PI / 2) * this.orbit.gravityCircle.radius
                );
            } else if (this.angle <= (3 * Math.PI) / 2) {
                this.sprite.setX(
                    this.orbit.sprite.x -
                    Math.cos(this.angle - Math.PI) * this.orbit.gravityCircle.radius
                );
                this.sprite.setY(
                    this.orbit.sprite.y +
                    Math.sin(this.angle - Math.PI) * this.orbit.gravityCircle.radius
                );
            } else if (this.angle < 2 * Math.PI) {
                this.sprite.setX(
                    this.orbit.sprite.x +
                    Math.cos(Math.PI * 2 - this.angle) * this.orbit.gravityCircle.radius
                );
                this.sprite.setY(
                    this.orbit.sprite.y +
                    Math.sin(Math.PI * 2 - this.angle) * this.orbit.gravityCircle.radius
                );
