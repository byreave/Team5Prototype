...
export default class Level {
	...
	createPlanetsFormJson(file) {
		...
		for (var i in data.planets) {
			...
			this.planets.push(
				this.planetGen.GenerateOnePlanet(x + this.centerPoint.x, y + this.centerPoint.y, radius, texture, name)
			);
		}
		for (var i in data.moons) {
			...
			if (m.isStartPosRandom) startPos = Math.random() * Math.PI * 2;
			var isCCW = m.isCCW;
			if (m.isCCWRandom) {
		...
		//Exits
		//Up
		if (data.exits.up.On == true && (this.levelUp == null || this.levelUp.exitsDown == null)) {
			//two levels share an exit, check
			this.exitsUp = new Exit(
				this.scene,
				this,
				'up',
				data.exits.up.PlanetTexture,
				data.exits.up.PlanetRadius,
				data.exits.up.MoonTexture,
				data.exits.up.isCCW
			);
		...
		if (data.exits.down.On == true && (this.levelDown == null || this.levelDown.exitsUp == null)) {
			this.exitsDown = new Exit(
				this.scene,
				this,
				'down',
				data.exits.down.PlanetTexture,
				data.exits.down.PlanetRadius,
				data.exits.down.MoonTexture,
				data.exits.down.isCCW
			);
		...
		if (data.exits.left.On == true && (this.levelLeft == null || this.levelLeft.exitsRight == null)) {
			this.exitsLeft = new Exit(
				...
				this,
				'left',
				data.exits.left.PlanetTexture,
				data.exits.left.PlanetRadius,
				data.exits.left.MoonTexture,
				data.exits.left.isCCW
			);
		...
		if (data.exits.right.On == true && (this.levelRight == null || this.levelRight.exitsLeft == null)) {
			this.exitsRight = new Exit(
				...
				this,
				'right',
				data.exits.right.PlanetTexture,
				data.exits.right.PlanetRadius,
				data.exits.right.MoonTexture,
				data.exits.right.isCCW
			);
